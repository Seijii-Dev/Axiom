...
import com.axiom.api.data.fs.installedPluginsJson
import com.axiom.api.data.fs.localBundleSourcesJson
...
import com.axiom.api.plugin.PluginRuntimeRegistry
import com.axiom.api.plugin.PluginRuntimeService
...
import com.axiom.core.koin
import com.axiom.data.file.archive.extractGzipTar
...
class PluginManager(
    ...
    private val runtimes = IdentityHashMap<AxiomPlugin, PluginRuntime>()
    ...
    suspend fun loadPluginBundle(
        ...
        val tmpDir = Paths.tempDir.resolve("axiom-plugin-bundles/tmp-${System.nanoTime()}")
        tmpDir.mkdirs()
        ...
        try {
            ...
            val backupDir = Paths.tempDir.resolve("axiom-plugin-bundles/backup-${System.nanoTime()}")
            ...
            try {
                ...
                val bundleCopy = File(pluginDir, "bundle.axiom")
    ...
    private fun instantiatePlugin(apkPath: String, desc: PluginDescriptor): AxiomPlugin {
        ...
        return instance as? AxiomPlugin
            ?: throw PluginLoadException("Class ${desc.entryClass} does not implement AxiomPlugin")
    }
    ...
    private suspend fun extractBundle(bundleUri: Uri, destination: File) = withContext(Dispatchers.IO) {
        val tmp = Paths.tempDir.resolve("axiom-plugin-bundles/bundles/${System.nanoTime()}.axiom")
        ...
        try {
            ...
            if (e.message == "Input is not in the .gz format.") {
                throw PluginLoadException("File is not in the .axiom format.")
            } else {
                throw e
            }
        } finally {
    ...
    private fun createPluginInfo(
        ...
        val apkFile = File(pluginDir, "plugin.apk")
        val bundleFile = pluginDir["bundle.axiom"]
    ...
    private suspend fun loadInstalledPlugins(progress: PluginLoadProgressListener? = null) =
        withContext(Dispatchers.IO) {
            ...
            // loaded (see AxiomPlugin.onStart docs: "all dependencies should be loaded and ready").
    ...
    override fun <T : PluginRuntimeService> service(plugin: AxiomPlugin, type: KClass<T>): T {
        synchronized(runtimes) {
