package com.axiom.api.service

import androidx.compose.runtime.Composable
import androidx.compose.runtime.remember
import com.axiom.api.data.log.LogEntry
import com.axiom.api.data.log.LogLevel
 ...
 * logging context, such as [AxiomPlugin]. For call sites with no meaningful
...
/**
 * Service responsible for logging events and messages within the Axiom ecosystem.
 *
 ...
 *    If you have a `AxiomPlugin` in context, you can use the concise extension:
 *    ```kotlin
 *    with(plugin) {
 ...
 *
 * The tag is derived from [ctx]: if it's a [AxiomPlugin], the plugin's descriptor
...
fun Logger.log(level: LogLevel, message: String, throwable: Throwable? = null) {
    val tag = when (ctx) {
        is AxiomPlugin -> ctx.info.descriptor.name
        NoLogContext -> "<anonymous>"
        else -> ctx::class.simpleName ?: "<anonymous>"
    }
    val sourcePluginId = (ctx as? AxiomPlugin)?.info?.id
