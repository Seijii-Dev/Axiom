...
class AxiomDescriptorGenerationExtension(
    session: FirSession,
    ...
    override fun getCallableNamesForClass(classSymbol: FirClassSymbol<*>, context: MemberGenerationContext): Set<Name> {
        ...
        if (key is Key) return setOf(AxiomPluginIds.DESCRIPTOR_PROPERTY_NAME, SpecialNames.INIT)
        ...
        if (classSymbol.isCompanion) {
            ...
            if (provider.matches(manifestPredicate, owner) && !classSymbol.declaresOwnDescriptor()) {
                return setOf(AxiomPluginIds.DESCRIPTOR_PROPERTY_NAME)
            }
        }
    ...
    override fun generateProperties(
        ...
        if (callableId.callableName != AxiomPluginIds.DESCRIPTOR_PROPERTY_NAME) return emptyList()
        val owner = context?.owner ?: return emptyList()

        val property = createMemberProperty(
            owner = owner,
            key = Key,
            name = AxiomPluginIds.DESCRIPTOR_PROPERTY_NAME,
    ...
    private fun FirClassSymbol<*>.declaresOwnDescriptor() = fir.declarations.any { decl ->
        decl.origin !is FirDeclarationOrigin.Plugin &&
                (decl as? FirCallableDeclaration)?.symbol?.callableId?.callableName == AxiomPluginIds.DESCRIPTOR_PROPERTY_NAME
    }
    ...
    private fun pluginDescriptorType() = ConeClassLikeTypeImpl(
        ConeClassLikeLookupTagImpl(AxiomPluginIds.PLUGIN_DESCRIPTOR_CLASS_ID),
