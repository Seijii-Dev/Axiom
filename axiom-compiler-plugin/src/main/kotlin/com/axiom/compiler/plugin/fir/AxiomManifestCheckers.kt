...
import com.axiom.compiler.plugin.AxiomPluginIds
...
object AxiomManifestErrors : KtDiagnosticsContainer() {
    ...
    val NOT_A_AXIOM_PLUGIN by error0<KtElement>()
    ...
    override fun getRendererFactory(): BaseDiagnosticRendererFactory = AxiomManifestDiagnosticRendererFactory()

    private class AxiomManifestDiagnosticRendererFactory : BaseDiagnosticRendererFactory() {
        ...
        override val MAP by KtDiagnosticFactoryToRendererMap("PluginManifest") { map ->
            map.apply {
                ...
                put(NOT_A_AXIOM_PLUGIN, "'@PluginManifest' can only annotate a class implementing AxiomPlugin")
                ...
                put(
                    ...
                    "Class ''{0}'' implements AxiomPlugin but is missing ''@PluginManifest(...)''. " +
                            "Every AxiomPlugin implementation must be annotated.",
                    rendererA = TO_STRING
                )
                put(
...
internal val manifestPredicate: LookupPredicate = LookupPredicate.create {
    annotated(AxiomPluginIds.PLUGIN_MANIFEST_FQN)
}

class AxiomManifestCheckers(session: FirSession) : FirAdditionalCheckersExtension(session) {

    override val declarationCheckers = object : DeclarationCheckers() {
        override val classCheckers: Set<FirClassChecker> = setOf(
            PluginManifestClassChecker,
            AxiomPluginImplementationChecker
        )
    }
...
private object AxiomPluginImplementationChecker : FirClassChecker(MppCheckerKind.Common) {

    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(declaration: FirClass) {
        val implementsAxiomPlugin = declaration.superConeTypes.any {
            it.classId == AxiomPluginIds.AXIOM_PLUGIN_CLASS_ID
        }
        if (!implementsAxiomPlugin) return

        val hasManifest = declaration.annotations.any {
            it.resolvedType.classId == AxiomPluginIds.PLUGIN_MANIFEST_CLASS_ID
        }

        if (!hasManifest) {
            val source = declaration.source ?: return
            reporter.reportOn(source, AxiomManifestErrors.MISSING_PLUGIN_MANIFEST, declaration.classId.shortClassName)
        }
    }
}
...
private object PluginManifestClassChecker : FirClassChecker(MppCheckerKind.Common) {
    ...
    override fun check(declaration: FirClass) {
        ...
        if (id == null || !AxiomPluginIds.isValidPluginId(id)) {
            reporter.reportOn(sourceFor("id"), AxiomManifestErrors.INVALID_PLUGIN_ID, id.orEmpty())
        }
        ...
        listOfNotNull(
            ...
            if (!AxiomPluginIds.isValidSemver(value)) {
                reporter.reportOn(sourceFor(label), AxiomManifestErrors.INVALID_SEMVER, label, value)
            }
        }
        ...
        if (
            ...
            AxiomPluginIds.isValidSemver(minAppVersion) &&
            AxiomPluginIds.isValidSemver(maxAppVersion)
        ) {
            if (compareSemver(minAppVersion, maxAppVersion) > 0) {
                reporter.reportOn(
                    sourceFor("minAppVersion"),
                    AxiomManifestErrors.MIN_VERSION_EXCEEDS_MAX,
                    minAppVersion,
                    maxAppVersion
                )
            }
        ...
        if (name.isNullOrBlank()) {
            reporter.reportOn(sourceFor("name"), AxiomManifestErrors.BLANK_NAME_FALLS_BACK_TO_ID, id)
        }
        ...
        if (!implementsAxiomPlugin) {
            reporter.reportOn(classSource, AxiomManifestErrors.NOT_A_AXIOM_PLUGIN)
        }
        ...
        if (declaration is FirRegularClass) {
            ...
            if (badModifiers.isNotEmpty()) {
                reporter.reportOn(
                    classSource, AxiomManifestErrors.INVALID_ENTRY_CLASS_MODIFIERS,
        ...
        val ownDescriptor = declaration.declarations
            ...
            ?.firstOrNull { it.symbol.callableId?.callableName == AxiomPluginIds.DESCRIPTOR_PROPERTY_NAME }

        if (ownDescriptor != null) {
            reporter.reportOn(ownDescriptor.source ?: classSource, AxiomManifestErrors.RESERVED_DESCRIPTOR_NAME)
        }
        ...
        if (!hasPublicNoArgConstructor) {
            reporter.reportOn(classSource, AxiomManifestErrors.MISSING_NO_ARG_CONSTRUCTOR)
        }
        ...
        if (allManifestClasses.size > 1) {
            ...
            reporter.reportOn(classSource, AxiomManifestErrors.DUPLICATE_PLUGIN_MANIFEST, others)
        }
    }
}
