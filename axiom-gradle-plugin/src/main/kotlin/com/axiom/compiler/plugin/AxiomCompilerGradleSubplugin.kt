...
import com.axiom.compiler.plugin.BuildConfig.AXIOM_API_LIBRARY_COORDINATES
...
@Suppress("unused")
class AxiomCompilerGradleSubplugin : KotlinCompilerPluginSupportPlugin {

    override fun apply(target: Project) {
        val pluginManager = target.pluginManager
        pluginManager.apply(SerializationGradleSubplugin::class.java)

        val extension = target.extensions.create("axiom", AxiomPluginExtension::class.java, target)

        extension.library.convention(false)
        extension.compose.convention(true)

        val pluginJson = target.layout.buildDirectory.file("axiom/generated/plugin.json")
        extension.outputFileName.convention(target.provider { target.rootProject.name })
        extension.outputDirectory.convention(target.layout.buildDirectory.dir("axiom"))
        ...
        val bundleDebug = target.tasks.register("axiomBundleDebug", Tar::class.java) { task ->
            task.group = "axiom"
            task.description = "Packages the debug variant into a valid .axiom distribution archive."
        ...
        val bundleRelease = target.tasks.register("axiomBundleRelease", Tar::class.java) { task ->
            task.group = "axiom"
            task.description = "Packages the release variant into a valid .axiom distribution archive."
        ...
        target.tasks.register("axiomBundle") { task ->
            task.group = "axiom"
            task.description = "Convenience alias targeting the axiomBundleRelease distribution task flow."
        ...
        listOf(
            ...
            val pushTask = target.tasks.register("axiomPush$variant") { task ->
                task.group = "axiom"
                ...
                task.doFirst {
                    ...
                    if (!jsonFile.exists()) {
                        throw GradleException(
                            "Axiom: no plugin descriptor found at '${jsonFile.absolutePath}'. " +
                                    "Annotate your AxiomPlugin implementation with @PluginManifest(...); " +
    ...
    override fun applyToCompilation(kotlinCompilation: KotlinCompilation<*>): Provider<List<SubpluginOption>> {
        ...
        val descriptorDir = project.layout.buildDirectory.dir("axiom/generated")

        kotlinCompilation.defaultSourceSet.dependencies {
            compileOnly(AXIOM_API_LIBRARY_COORDINATES)
        }
    ...
    private fun setupBaseTarProperties(task: Tar, extension: AxiomPluginExtension) {
        task.archiveExtension.set("axiom")
    ...
    private fun pushBundleToDevice(project: Project, bundleFile: File) {
        ...
        try {
            ...
            if (devices.size == 1) {
                ...
                project.logger.lifecycle("Axiom: Pushing ${bundleFile.name} to device $deviceId...")
                try {
                    project.providers.exec { spec ->
                        spec.commandLine(adbExecutable, "-s", deviceId, "shell", "mkdir", "-p", "/sdcard/axiom/plugins/")
                    }.result.get()
                    project.providers.exec { spec ->
                        spec.commandLine(
                            adbExecutable,
                            ...
                            "/sdcard/axiom/plugins/"
                        )
                    }.result.get()
                } catch (e: Exception) {
                    project.logger.error("Axiom: Failed to push bundle to device: ${e.message}")
                }
            } else if (devices.size > 1) {
                project.logger.warn("Axiom: Multiple devices connected, skipping auto-push.")
            }
        } catch (e: Exception) {
            project.logger.warn("Axiom: Failed to run adb. Is it in your PATH?", e)
        }
    }
}
