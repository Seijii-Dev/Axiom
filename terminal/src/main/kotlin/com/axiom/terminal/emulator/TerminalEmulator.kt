package com.axiom.terminal.emulator

import androidx.compose.runtime.getValue
import androidx.compose.runtime.mutableStateOf
import androidx.compose.runtime.setValue
import com.axiom.terminal.Logger
import com.axiom.terminal.emulator.TerminalEmulator.Companion.DECSET_BIT_CURSOR_ENABLED
import com.axiom.terminal.emulator.TerminalEmulator.Companion.DECSET_BIT_ORIGIN_MODE
import com.axiom.terminal.emulator.TerminalEmulator.Companion.ESC
import com.axiom.terminal.emulator.TerminalEmulator.Companion.ESC_APC
import com.axiom.terminal.emulator.TerminalEmulator.Companion.ESC_CSI
import com.axiom.terminal.emulator.TerminalEmulator.Companion.ESC_CSI_QUESTIONMARK
import com.axiom.terminal.emulator.TerminalEmulator.Companion.ESC_CSI_UNSUPPORTED_INTERMEDIATE_BYTE
import com.axiom.terminal.emulator.TerminalEmulator.Companion.ESC_CSI_UNSUPPORTED_PARAMETER_BYTE
import com.axiom.terminal.emulator.TerminalEmulator.Companion.ESC_P
import com.axiom.terminal.emulator.TerminalEmulator.Companion.mapDecSetBitToInternalBit
...
class TerminalEmulator(
     ...
     */
    private val mainBuffer =
        TerminalBuffer(columns, getTerminalTranscriptRows(transcriptRows), rows)
    ...
    val isCursorKeysApplicationMode
        ...
        when (b) {
            ...
            27 -> doEscapeKey(b)
                ...
                when (escapeState) {
                    ...
                    ESC_CSI_UNSUPPORTED_INTERMEDIATE_BYTE -> doCsiUnsupportedParameterOrIntermediateByte(
                        b
                    )
    ...
    private fun setCursorPosition(x: Int, y: Int) {
        ...
        val effectiveRightMargin = if (originMode) rightMargin else columns
        val newRow =
            maxOf(effectiveTopMargin, minOf(effectiveTopMargin + y, effectiveBottomMargin - 1))
        val newColumn =
            maxOf(effectiveLeftMargin, minOf(effectiveLeftMargin + x, effectiveRightMargin - 1))
    ...
    private fun emitCodePoint(codePoint: Int) {
        ...
        val offsetDueToCombiningChar =
            if (displayWidth <= 0 && cursorColumn > 0 && !aboutToAutoWrap) 1 else 0
    ...
    private fun unimplementedSequence(b: Int) {
        logError(
            "Unimplemented sequence char '${b.toChar()}' (U+${
                b.toUInt().toString(16).padStart(4, '0')
            })"
        )
        finishSequence()
    }
