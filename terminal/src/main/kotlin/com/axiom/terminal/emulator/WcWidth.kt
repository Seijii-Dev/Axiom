 ...
 *
 * Implementation from https://github.com/termux/termux-app/blob/master/terminal-emulator/src/main/java/com/termux/terminal/WcWidth.java.
 */
object WcWidth {
    ...
    fun width(ucs: Int): Int {
        if (ucs == 0 || ucs == 0x034F ||
            (ucs in 0x200B..0x200F) || ucs == 0x2028 || ucs == 0x2029 ||
        ...
        if (ucs < 32 || (ucs in 0x07F..<0x0A0)) return 0
    ...
    fun width(chars: CharArray, index: Int): Int {
        val c = chars[index]
        return if (c.isHighSurrogate()) width(Character.toCodePoint(c, chars[index + 1])) else width(c.code)
    }
