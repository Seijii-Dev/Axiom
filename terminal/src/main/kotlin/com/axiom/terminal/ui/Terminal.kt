...
import com.axiom.terminal.ScreenEvent
import com.axiom.terminal.emulator.CursorStyle
import com.axiom.terminal.emulator.TerminalEmulator
import com.axiom.terminal.emulator.TerminalSession
import com.axiom.terminal.emulator.TerminalSessionClient
import com.axiom.terminal.ui.selection.TextSelectionOverlay
import com.axiom.terminal.ui.selection.TextSelectionState
import com.axiom.terminal.ui.selection.icon.TextSelectHandleLeft
import com.axiom.terminal.ui.selection.icon.TextSelectHandleRight
...
internal suspend fun PlatformTextInputSessionScope.createInputRequest(state: TerminalState): PlatformTextInputMethodRequest {
    ...
    return PlatformTextInputMethodRequest { outAttrs ->
        ...
        outAttrs.inputType = resolveInputType(client)
        ...
        object : BaseInputConnection(view, true) {
            ...
            suspend fun sendTextToTerminal(text: CharSequence) {
                ...
                while (index < length) {
                    ...
                    val (processedCodePoint, ctrlHeld) = applySoftKeyboardControlCode(codePoint)
                    state.inputCodePoint(KeyEventSource.SoftKeyboard, processedCodePoint, ctrlHeld, false)
                }
            }
        }
    }
...
/**
 * Resolve the [InputType] to use for the terminal based on the selection state.
 * See [createInputRequest] for the rationale.
 */
private fun resolveInputType(client: TerminalClient): Int {
    if (client.isTerminalSelected) {
        if (client.shouldEnforceCharBasedInput) {
            ...
            return InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD or InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS
        }
...
/**
 * Map a soft-keyboard code point to what the terminal expects, translating control characters
 * (e.g. a newline to a carriage return, or `ctrl+letter` encodings) into a (codePoint, ctrlHeld) pair.
 */
private fun applySoftKeyboardControlCode(codePoint: Int): Pair<Int, Boolean> {
    if (codePoint > 31 || codePoint == 27) {
        return codePoint to false
    }

    var processedCodePoint = codePoint
    if (processedCodePoint == '\n'.code) {
        // The AOSP keyboard and descendants seems to send \n as text when the enter key is pressed,
        // instead of a key event like most other keyboard apps. A terminal expects \r for the enter
        // key (although when icrnl is enabled this doesn't make a difference - run 'stty -icrnl' to
        // check the behaviour).
        processedCodePoint = '\r'.code
    }

    // E.g. penti keyboard for ctrl input.
    val ctrlHeld = true
    processedCodePoint = when (processedCodePoint) {
        31 -> '_'.code
        30 -> '^'.code
        29 -> ']'.code
        28 -> '\\'.code
        else -> processedCodePoint + 96
    }
    return processedCodePoint to ctrlHeld
}
