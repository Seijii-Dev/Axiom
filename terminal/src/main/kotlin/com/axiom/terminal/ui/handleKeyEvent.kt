...
import com.axiom.terminal.emulator.KeyHandler
import com.axiom.terminal.emulator.KeyMod
import com.axiom.terminal.emulator.TermKey
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.launch
import android.view.KeyEvent as AndroidKeyEvent

internal fun TerminalState.handleKeyEvent(
    event: KeyEvent,
    ...
    when (event.type) {
        KeyEventType.KeyDown -> {
            ...
            if (enableKeyLogging) {
                client.logInfo(
                    TerminalState.LOG_TAG,
                    "[Terminal KeyEvent] resolvedKey=$resolvedKey"
                )
            }
...
internal fun TerminalState.handleKey(
    key: Key,
    keyMod: Int,
    coroutineScope: CoroutineScope
): Boolean {
    ...
    val emulator =
        checkNotNull(session.emulator) { "TerminalEmulator should not be null at this point." }
...
internal fun KeyEvent.resolveUnicodeKey(client: TerminalClient): ResolvedKey? {
